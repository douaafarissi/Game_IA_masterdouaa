  ...
  */
  ...
  class Vehicle {
    ...
    constructor(x, y, imageVaisseau) {
      //wander
       // Paramètres spécifiques au comportement wander
    ...
    wander() {
      ...
      // Calcul du point sur le cercle
      let theta = this.wanderTheta + this.vel.heading();
      let x = this.wanderRadius * cos(theta);
      let y = this.wanderRadius * sin(theta);
      wanderPoint.add(x, y);
  
      // Calcul de la "desiredSpeed" (vitesse désirée)
      let desiredSpeed = wanderPoint.copy().sub(this.pos);
  
      // Force à appliquer au véhicule (équivalent à la "desiredSpeed")
      let force = desiredSpeed;
      force.setMag(this.maxForce);
      this.applyForce(force);
  
      // Déplacement du point sur le cercle
    ...
    applyBehaviors(target, obstacles, vehicules, distance) {
      let seekForce = this.arrive(target, true, distance);
  ...
  avoid(obstacles) {
    ...
    // On calcule la distance entre le centre du rectangle de l'obstacle 
    // et le bout du vecteur ahead
    let distance = this.distToRect(pointAuBoutDeAhead, obstacleLePlusProche);
    // et pour ahead2
    let distance2 = this.distToRect(pointAuBoutDeAhead2, obstacleLePlusProche);
    // et pour la position du vaisseau
    let distance3 = this.distToRect(this.pos, obstacleLePlusProche);
    ...
    if (distance == plusPetiteDistance) {
      ...
      if (distance3 < obstacleLePlusProche.width / 2 && distance3 < obstacleLePlusProche.height / 2) {
        alerteRougeVaisseauDansObstacle = true;
        obstacleLePlusProche.color = color("red");
      } else {
        obstacleLePlusProche.color = color(0, 255, 0);
      }
    }
    ...
    if (Vehicle.debug) {
      ...
      line(this.pos.x, this.pos.y, pointAuBoutDeAhead.x, pointAuBoutDeAhead.y, this.rayonZoneDeFreinage);
    }
  
    // si la distance est < rayon de l'obstacle
    if (plusPetiteDistance < max(obstacleLePlusProche.width / 2, obstacleLePlusProche.height / 2) + this.largeurZoneEvitementDevantVaisseau) {
      // collision possible
  ...
  distToRect(point, obstacle) {
    let dx = abs(point.x - obstacle.pos.x);
    let dy = abs(point.y - obstacle.pos.y);
    let distanceX = max(dx - obstacle.width / 2, 0);
    let distanceY = max(dy - obstacle.height / 2, 0);
    ...
    arrive(target, arrival=false, distanceVisee = 0) {
